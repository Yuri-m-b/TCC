...
static int16_t zb_pressure_to_s16(float hpa)
{
    return (int16_t)(hpa * 10);
}
...
void update_pressure_value(float hpa)
{
    int16_t pressure_value = (int16_t)(hpa * 10);  // décimos de hPa

    esp_zb_lock_acquire(portMAX_DELAY);
    esp_zb_zcl_set_attribute_val(HA_ESP_PRESSURE_SENSOR_ENDPOINT,
...
static esp_zb_ep_list_t *custom_pressure_sensor_ep_create(uint8_t endpoint_id, esp_zb_pressure_meas_cluster_cfg_t *pressure_sensor)
{
    esp_zb_ep_list_t *ep_list = esp_zb_ep_list_create();
    esp_zb_endpoint_config_t endpoint_config = {
        .endpoint = endpoint_id,
        .app_profile_id = ESP_ZB_AF_HA_PROFILE_ID,
        .app_device_id = ESP_ZB_HA_PRESSURE_SENSOR_DEVICE_ID, // YURI CRIOU
        .app_device_version = 0
    };
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_pressure_meas_cluster_cfg_t sensor_cfg2 = ESP_ZB_DEFAULT_PRESSURE_SENSOR_CONFIG();
    /* Set (Min|Max)MeasuredValure */
    sensor_cfg2.pressure_meas_cfg.min_value = zb_pressure_to_s16(ESP_ZB_ZCL_ATTR_PRESSURE_MEASUREMENT_MIN_VALUE_MIN_VALUE);
    sensor_cfg2.pressure_meas_cfg.max_value = zb_pressure_to_s16(ESP_ZB_ZCL_ATTR_PRESSURE_MEASUREMENT_MIN_VALUE_MAX_VALUE);
    esp_zb_ep_list_t *esp_zb_sensor2_ep = custom_pressure_sensor_ep_create(HA_ESP_PRESSURE_SENSOR_ENDPOINT, &sensor_cfg2);

    /* Register the device */
    esp_zb_device_register(esp_zb_sensor_ep);
    esp_zb_device_register(esp_zb_sensor2_ep);
